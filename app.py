...
# ── Startup guards - fail with a clear message, not a stack trace ────────────
...
from bot import ask
from retriever import get_collection_size
from quiz import (
    get_weighted_chunks,
    generate_question,
    calculate_score,
...
if get_collection_size() == 0:
    st.error(
        "The lore database is empty. Run `python ingest.py --reset --validate` "
        "to build it, then restart the app."
    )
    st.stop()
...
.stApp {
    ...
    background-image:
        radial-gradient(ellipse 90% 45% at 50% -8%, #1a1200 0%, transparent 65%),
        radial-gradient(ellipse 55% 32% at 82% 108%, #120c00 0%, transparent 70%),
        radial-gradient(ellipse 45% 28% at 12% 104%, #0c0900 0%, transparent 70%);
    color: #c9a84c;
}
/* Floating motes of grace drifting upward */
.stApp::before {
    content: "";
    position: fixed;
    inset: 0;
    ...
    pointer-events: none;
    background-image:
        radial-gradient(1.6px 1.6px at 84px 546px, #e8c87088, transparent 100%),
        radial-gradient(1.2px 1.2px at 196px 238px, #c9a84c66, transparent 100%),
        radial-gradient(2px 2px at 308px 434px, #e8c87055, transparent 100%),
        radial-gradient(1.3px 1.3px at 427px 147px, #c9a84c55, transparent 100%),
        radial-gradient(1.7px 1.7px at 525px 497px, #e8c87077, transparent 100%),
        radial-gradient(1.2px 1.2px at 623px 308px, #c9a84c55, transparent 100%),
        radial-gradient(1.5px 1.5px at 364px 616px, #e8c87044, transparent 100%),
        radial-gradient(1.2px 1.2px at 56px 105px, #c9a84c44, transparent 100%);
    background-repeat: repeat;
    background-size: 700px 700px;
    animation: motes-drift 100s linear infinite;
    opacity: 0.65;
}
...
.stApp::after {
    ...
    z-index: 0;
    pointer-events: none;
    background: radial-gradient(ellipse 130% 95% at 50% 40%, transparent 60%, #00000077 100%);
}
...
.eldenlore-title {
    ...
    line-height: 1.2;
    display: block;
    background: linear-gradient(180deg, #f6e7b2 0%, #e0bf6a 45%, #c9a84c 65%, #96742e 100%);
    -webkit-background-clip: text;
    background-clip: text;
    -webkit-text-fill-color: transparent;
    filter: drop-shadow(0 0 26px #c9a84c40) drop-shadow(0 2px 3px #000000cc);
}
.eldenlore-sub {
...
.grace-diamond {
    ...
    flex-shrink: 0;
    animation: grace-pulse 3s ease-in-out infinite;
}
...
.stButton > button:hover {
    background: linear-gradient(180deg, #161100 0%, #0f0b00 100%) !important;
    color: #e8c870 !important;
    border-color: #c9a84c77 !important;
    box-shadow: 0 0 18px #c9a84c26, inset 0 1px 0 #c9a84c1a !important;
    transform: translateY(-1px) !important;
}

/* ── Begin the Trial button (styled via Streamlit's key class) ── */
.st-key-start_quiz button {
    font-family: 'Cinzel', serif !important;
...
.st-key-start_quiz button:hover {
    border-color: #c9a84c !important;
    box-shadow: 0 0 20px #c9a84c33 !important;
}
...
/* Hide default avatars - the character speaker headers replace them */
...
[data-testid="stChatMessage"]:has([data-testid="stChatMessageAvatarAssistant"]) [data-testid="stChatMessageContent"] {
    background: linear-gradient(160deg, #100d02 0%, #0a0900 55%, #0c0a01 100%) !important;
    border: 1px solid #c9a84c2e !important;
    border-left: 2px solid #c9a84c88 !important;
    color: #e8d5a3 !important;
    font-family: 'Crimson Text', serif !important;
    ...
    box-shadow: 0 0 28px #c9a84c0d, inset 0 1px 0 #c9a84c14 !important;
}
...
.score-card::after {
    ...
    width: 40%;
    height: 100%;
    background: linear-gradient(100deg, transparent, #f6e7b214, transparent);
...
/* ── Finishing touches ── */
/* Centre the sidebar navigation entries */
...
@keyframes grace-pulse {
    0%, 100% { box-shadow: 0 0 8px #c9a84c66;  opacity: 0.8; }
    50%      { box-shadow: 0 0 18px #e8c870cc; opacity: 1; }
}
@keyframes rise-in {
...
@keyframes rune-breathe {
    0%, 100% { opacity: 0.55; text-shadow: 0 0 10px #c9a84c22; }
    50%      { opacity: 1;    text-shadow: 0 0 26px #c9a84c55; }
}
...
@keyframes rank-glow {
    0%, 100% { text-shadow: 0 0 14px #c9a84c55; }
    50%      { text-shadow: 0 0 30px #e8c870aa; }
}
</style>
...
with st.sidebar:
    ...
    if st.button("Clear Audience", key="clear_btn", use_container_width=True):
    ...
    if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        ...
        with st.chat_message("assistant"):
            with st.spinner("The Lands Between stir..."):
                answer, sources, mode, char_name = ask(question, st.session_state.messages[:-1])
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.quiz_started:
        ...
        with col2:
            if st.button("Begin the Trial", key="start_quiz", use_container_width=True):
                with st.spinner("The trial is being prepared…"):
                    # 4× candidate pool so generation failures don't leave us
                    # short, plus cross-session history so questions don't repeat.
                    chunks = get_weighted_chunks(count=40)
                    history_list = load_history_list()
                    seen_hashes = set(history_list)
                    questions = []
                ...
                if not questions:
                    st.error(
                        "The trial could not be prepared - the question service "
    ...
    # ── QUESTION SCREEN ───────────────────────────────────────────────────
        ...
        if not questions:
            ...
            st.rerun()
        current = min(st.session_state.current_q, len(questions) - 1)
        total = len(questions)
        q = questions[current]
            ...
            if selected:
                ...
                if current < total - 1:
                    ...
                    if answered >= total:
                        ...
                        st.button("Submit ✦", key="submit_disabled", disabled=True, use_container_width=True)
    ...
    # ── RESULTS SCREEN ────────────────────────────────────────────────────
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Face the Trial Again", key="retry", use_container_width=True):
                st.session_state.quiz_started = False
                ...
                st.rerun()
        with col2:
            if st.button("Return to Gideon", key="return", use_container_width=True):
                st.session_state.quiz_started = False
                st.session_state.quiz_questions = []
                st.session_state.selected_answers = {}
                st.session_state.quiz_submitted = False
