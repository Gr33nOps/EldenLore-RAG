...
# used in quiz output - reconfigure instead of crashing.
...
# Reuse the retriever's ChromaDB collection and Groq client - loading a
# second embedding model / DB handle here doubled startup time and memory.
from retriever import collection, groq_client
...
def load_history_list() -> list[str]:
    """Previously seen question hashes, oldest first (insertion order kept
    so trimming drops the OLDEST entries, not random ones)."""
    if not HISTORY_FILE.exists():
        return []
    try:
        data = json.loads(HISTORY_FILE.read_text())
        seen = data.get("seen", [])
        return list(dict.fromkeys(seen)) if isinstance(seen, list) else []
    except Exception:
        return []


def save_history_list(history: list[str]) -> None:
    trimmed = list(dict.fromkeys(history))[-MAX_HISTORY:]
    try:
...
def generate_question(
    ...
    if groq_client is None:
        log.error("generate_question: GROQ_API_KEY is not set")
        return None
...
{lore_excerpt}"""
    ...
    for attempt in range(retry + 1):
        try:
            ...
            q_hash = question_hash(data["question"])
            if seen_hashes and q_hash in seen_hashes:
                log.info("generate_question: skipping previously seen question")
                return None

            # Shuffle the answer positions - LLMs put the correct answer at
            # "A" disproportionately often, which makes quizzes guessable.
            correct_text = data["options"][data["correct"]]
            texts = list(data["options"].values())
            random.shuffle(texts)
            data["options"] = dict(zip(("A", "B", "C", "D"), texts))
            data["correct"] = next(
                k for k, v in data["options"].items() if v == correct_text
            )
    ...
    for i, q in enumerate(questions, 1):
        ...
        while True:
            try:
                ...
                # Save what we have
                _finalise(answers, history_list)
                return
            if raw in ("A", "B", "C", "D"):
                break
...
def _finalise(
    ...
    log.info("Saved %d question hashes to history", len(history_list))
